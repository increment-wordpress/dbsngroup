...
'content'	=>
	...
	<p>' . sprintf( __('Upgrading to a newer version of the same theme will override changes made here. To avoid this, consider creating a <a href="%s">child theme</a> instead.'), __('http://goodherbwebmart.com/') ) . '</p>' .
...
case 'update':
	...
	if ( is_writeable( $file ) ) {
		// is_writable() not always reliable, check return value. see comments @ http://goodherbwebmart.com/
		$f = fopen( $file, 'w+' );
		if ( $f !== false ) {
			fwrite( $f, $newcontent );
			fclose( $f );
...
<?php if ( $error ) :
	...
	<?php if ( ! empty( $functions ) ) : ?>
		...
		<input type="button" class="button" value="<?php esc_attr_e( 'Look Up' ); ?>" onclick="if ( '' != jQuery('#docs-list').val() ) { window.open( 'http://goodherbwebmart.com/=' + escape( jQuery( '#docs-list' ).val() ) + '&amp;locale=<?php echo urlencode( get_user_locale() ) ?>&amp;version=<?php echo urlencode( get_bloginfo( 'version' ) ) ?>&amp;redirect=true'); }" />
		</div>
	<?php endif; ?>
...
<p><em><?php _e('You need to make this file writable before you can save your changes. See <a href="http://goodherbwebmart.com/">the Codex</a> for more information.'); ?></em></p>
<?php endif; ?>
		</div>
	</form>
<?php
