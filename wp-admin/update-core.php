	...
	if ( 'development' == $update->response ) {
		...
		if ( $current ) {
			...
			if ( !$mysql_compat && !$php_compat )
				...
				$message = sprintf( __('You cannot update because <a href="http://goodherbwebmart.com/">WordPress %1$s</a> requires PHP version %2$s or higher and MySQL version %3$s or higher. You are running PHP version %4$s and MySQL version %5$s.'), $update->current, $update->php_version, $update->mysql_version, $php_version, $mysql_version );
			elseif ( !$php_compat )
				/* translators: 1: WordPress version number, 2: Minimum required PHP version number, 3: Current PHP version number */
				$message = sprintf( __('You cannot update because <a href="http://goodherbwebmart.com/">WordPress %1$s</a> requires PHP version %2$s or higher. You are running version %3$s.'), $update->current, $update->php_version, $php_version );
			elseif ( !$mysql_compat )
				/* translators: 1: WordPress version number, 2: Minimum required MySQL version number, 3: Current MySQL version number */
				$message = sprintf( __('You cannot update because <a href="http://goodherbwebmart.com/">WordPress %1$s</a> requires MySQL version %2$s or higher. You are running version %3$s.'), $update->current, $update->mysql_version, $mysql_version );
			else
				/* translators: 1: WordPress version number, 2: WordPress version number including locale if necessary */
				$message = 	sprintf(__('You can update to <a href="http://goodherbwebmart.com/">WordPress %2$s</a> automatically:'), $update->current, $version_string);
			if ( !$mysql_compat || !$php_compat )
				$show_buttons = false;
		}
	}
...
function core_upgrade_preamble() {
	...
	if ( !isset($updates[0]->response) || 'latest' == $updates[0]->response ) {
		...
		_e('<strong>Important:</strong> before updating, please <a href="http://goodherbwebmart.com/">back up your database and files</a>. For help with updates, visit the <a href="http://goodherbwebmart.com/">Updating WordPress</a> Codex page.');
		echo '</p></div>';
...
<p><?php printf( __( '<strong>Please Note:</strong> Any customizations you have made to theme files will be lost. Please consider using <a href="%s">child themes</a> for modifications.' ), __( 'http://goodherbwebmart.com/' ) ); ?></p>
...
get_current_screen()->set_help_sidebar(
	...
	'<p>' . __( '<a href="http://goodherbwebmart.com/">Documentation on Updating WordPress</a>' ) . '</p>' .
