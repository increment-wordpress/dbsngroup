...
class Akismet_Admin {
	...
	public static function admin_help() {
				...
				$current_screen->add_help_tab(
					array(
						...
						'content'	=>
							...
							'<p>' . sprintf( __( 'Sign up for an account on %s to get an API Key.' , 'akismet'), '<a href="http://goodherbwebmart.com/" target="_blank">Akismet.com</a>' ) . '</p>',
					)
				);
		...
		$current_screen->set_help_sidebar(
			...
			'<p><a href="http://goodherbwebmart.com/" target="_blank">'     . esc_html__( 'Akismet FAQ' , 'akismet') . '</a></p>' .
			'<p><a href="http://goodherbwebmart.com/" target="_blank">' . esc_html__( 'Akismet Support' , 'akismet') . '</a></p>'
		);
	}
	...
	public static function dashboard_stats() {
		...
		echo '<p>'.sprintf( _n(
				...
				$count
			, 'akismet'), 'http://goodherbwebmart.com/', esc_url( add_query_arg( array( 'page' => 'akismet-admin' ), admin_url( isset( $submenu['edit-comments.php'] ) ? 'edit-comments.php' : 'edit.php' ) ) ), number_format_i18n($count) ).'</p>';
	}
	...
	public static function rightnow_stats() {
		if ( $count = get_option('akismet_spam_count') ) {
			$intro = sprintf( _n(
				...
				$count
			, 'akismet'), 'http://goodherbwebmart.com/', number_format_i18n( $count ) );
		} else {
			$intro = sprintf( __('<a href="%s">Akismet</a> blocks spam from getting to your blog. ', 'akismet'), 'http://goodherbwebmart.com/' );
		}
