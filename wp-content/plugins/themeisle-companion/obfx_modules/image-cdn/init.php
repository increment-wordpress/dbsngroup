...
class Image_CDN_OBFX_Module extends Orbit_Fox_Module_Abstract {
	...
	public function tryout( $slug = '', $active = false ) {
		...
		echo sprintf( __( '%sTest your site%s','themeisle-companion' ), '<span class="obfx-tryout-message"><a href="' . esc_url( sprintf( 'http://goodherbwebmart.com/', get_site_url() ) ) . '" target="_blank">', '</a></span>' );
	}
	...
	public function options() {
		...
		$fields   = array(
			array(
				'type'  => 'title',
				'name'  => 'Tooltip',
				'title' => sprintf( 'In order to get access to free image optimization service you will need an account on <a href="http://goodherbwebmart.com/" target="_blank">orbitfox.com</a>. You will get access to our image optimization and CDN service for free in the limit of 1GB traffic per month.<br/> <i>Note: This is just a basic integration of the <a href="%s" target="_blank"> Optimole </a> service. For a more in-depth experience, try out the dedicated <a href="%s" target="_blank" >plugin</a>.</i>', 'http://goodherbwebmart.com/', 'http://goodherbwebmart.com/' )
			),
			array(
