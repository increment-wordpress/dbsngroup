 ...
 * Project Home Page: http://goodherbwebmart.com/
 *
...
if ( !class_exists('phpFlickr') ) {
	...
	class phpFlickr {
		...
		var $rest_endpoint = 'http://goodherbwebmart.com/';
		var $upload_endpoint = 'http://goodherbwebmart.com/';
		var $replace_endpoint = 'http://goodherbwebmart.com/';
		var $req;
		var $response;
		var $parsed_response;
		var $cache = false;
		...
		function __construct ($api_key, $secret = NULL, $die_on_error = false) {
			...
			//get your own at http://goodherbwebmart.com/
...
/* 			$this->parsed_response = $this->clean_text_nodes(json_decode($this->response, TRUE)); */
		...
		function auth ($perms = "read", $remember_uri = true) {
			...
			if (empty($_SESSION['phpFlickr_auth_token']) && empty($this->token)) {
				...
				if ($this->service == "23") {
					header("Location: http://goodherbwebmart.com/=" . $this->api_key . "&perms=" . $perms . "&api_sig=". $api_sig);
				} else {
					header("Location: http://goodherbwebmart.com/=" . $this->api_key . "&perms=" . $perms . "&api_sig=". $api_sig);
				}
				exit;
		...
		function auth_url($frob, $perms = 'read') {
			...
			return sprintf('http://goodherbwebmart.com/', $this->api_key, $perms, $frob, $sig);
		}
